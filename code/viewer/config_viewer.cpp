...
ConfigViewer::ConfigViewer (void)
{
	...
	projector.topBottomRatio = 1.5;
	projector.showCalibrationBars = false;
	projector.yGama = 1.0;
	projector.calibBarsCount = 10;
	projector.angle = 0;
...
void ConfigViewer::Load(QString path)
{
	appPath = path;
	iniFile = path + "/cassapa_viewer.ini";
	input.terminate = 0;
	...
	isMainController = settings.value("isMainController", isMainController).toBool();
	settings.endGroup();
	...
	input.initialVideoPos = settings.value("initialVideoPos", 0).toInt();
	input.finalVideoPos = settings.value("finalVideoPos", 0).toInt();
	
	light = settings.value("light", "1.0").toFloat();
	showCue = settings.value("showCue", false).toBool();
	sound.volume = settings.value("sound.volume", sound.volume).toFloat();
	...
	projector.box.x1 = settings.value("projector.X1", 0).toFloat();
	projector.box.y1 = settings.value("projector.Y1", 0).toFloat();
	projector.box.x2 = settings.value("projector.X2", 640).toFloat();
	projector.box.y2 = settings.value("projector.Y2", 480).toFloat();

	projector.topBottomRatio = settings.value("projector.topBottomRatio", projector.topBottomRatio).toFloat();
	projector.showCalibrationBars = settings.value("projector.showCalibrationBars", projector.showCalibrationBars).toBool();
	projector.yGama = settings.value("projector.yGama", projector.yGama).toFloat();
	projector.calibBarsCount = settings.value("projector.calibBarsCount", projector.calibBarsCount).toInt();
	projector.angle = settings.value("projector.angle", projector.angle).toInt();
...
void ConfigViewer::Save(void)
{
	...
	settings.setValue("isMainController", isMainController);
	settings.endGroup();
	...
	settings.setValue("light",  QString::number(light,'f',5));
	//settings.setValue("cueHistorySize",  cueHistorySize);
	settings.setValue("showCue", showCue);
